...
class SiteHelpers
{
    public static function menus($position = 'top', $active = '1')
    {
        if (empty(Session::get('uid'))) {
            return abort(403, 'Обнаружен не санкционированный доступ!');
        }
        $data = array();
        $menu = self::nestedMenu(0, $position, $active);
        $unread = \DB::select('select (select count(*) from tb_news limit 1) - (select count(*) from tb_news_read where id_user=' . Session::get('uid', 0) . ') as unread limit 1');
        if ($unread) $unread = $unread[0]->unread;
        foreach ($menu as $row) {
            $child_level = array();
            ...
            if ($row->allow_guest == 1) {
                ...
                $is_allow = (isset($p[Session::get('gid', 0)]) && $p[Session::get('gid', 0)] ? 1 : 0);
            }
            if ($is_allow == 1) {
                    ...
                    foreach ($menus2 as $row2) {
                        ...
                        if ($is_allow == 1) {
                            $menu2 = array(
                                ...
                                'module' => ($row2->module == 'listovka2' ? 'listovka' : $row2->module),
                                'menu_type' => ($row2->module == 'listovka2' ? 'external' : $row2->menu_type),
                                'url' => ($row2->module == 'listovka2' ? 'listovka/indexout' : $row2->url),
                ...
                if ($row->module == 'news') $level['unread'] = $unread;
                $data[] = $level;
            }
        }
    ...
    {
        $pass = $pass ? $pass : Config::get('app.key') . \Session::get('_token');
    ...
    public static function gridClass($layout, $block = '')
    {
    ...
    static function accessDenied($err = 403)
    {
        switch ($err) {
            case 404:
                return "<img src='" . asset("uploads/images/err_404.png") . "' width='212px;' style='display: block;margin-left: auto;margin-right: auto;width: 50%;'><br><span style='font-weight: 700; color:#FF0000;font-size: 1.1em;text-align: center;display: block;margin-left: auto;margin-right: auto;width: 50%;'>" . \Lang::get('core.note_noexists') . "</span>";
                break;
            default:
                return "<img src='" . asset("uploads/images/err_403.png") . "' width='212px;' style='display: block;margin-left: auto;margin-right: auto;width: 50%;'><br><span style='font-weight: 700; color:#FF0000;font-size: 1.1em;text-align: center;display: block;margin-left: auto;margin-right: auto;width: 50%;'>" . \Lang::get('core.note_restric') . "</span>";
        }
    }
    ...
    public static function formatRows($value, $attr, $row = null)
    {
        ...
        } else if ($format_as == 'date' and !empty($value)) {
            // FORMAT AS DATE
            if ($format_value == '') {
    ...
    {
        $avatar = '<img alt="" src="' . asset("uploads/users/__no_photo__.png") . '" class="img-circle" width="' . $width . '" />';
    ...
    public static function BBCode2Html($text)
    {
        ...
        // And replace them by...
        $out = array(
            '<img alt=":)" src="' . $emotion . 'emoticon-happy.png" />',
    ...
    public static function auditAction($request, $data)
    {
        $data = [
            'module' => $data['module'],
            'id_row' => $data['id_row'],
        ...
        $db = (env('THIS_SERVER', "AXMAT_SILA") != 'MVD') ? \DB::connection('mysql_02') : \DB::connection('mysql');
        $db->table('tb_audit')->insert($data);
    }
